...
async function generateEstimate({ name, company, budget, timeline, brief }) {
  ...
  try {
    const report = await callGemini(primary, prompt);
    return { report, model: primary };
  } catch {
    try {
      const report = await callGemini(fallback, prompt);
