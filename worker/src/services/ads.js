export const PLACEMENTS = ['blog_hero', 'post_sidebar_a', 'post_sidebar_b'];

export function isValidPlacement(p) {
  return PLACEMENTS.includes(p);
}

export function isAdLive(ad, now = new Date()) {
...
export function pickLiveAd(rows) {
...
export function mapPublicAd(row) {
  if (!row) return null;
  return {
    id: row.id,
...
export function mapAdminAd(row) {
  if (!row) return null;
  return {
    id: row.id,
    placement: row.placement,
...
export function buildAdsByPlacement(rows) {
  const grouped = {};
...
export function latestAdPerPlacement(rows) {
  const out = {};
...
export function normalizeAdBody(body) {
